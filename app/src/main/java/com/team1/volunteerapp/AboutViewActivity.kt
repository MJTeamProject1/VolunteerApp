...
class AboutViewActivity : AppCompatActivity() {
                        ...
                        for (doc in result) {
                            if (doc["uid"] == auth.uid.toString()) {
                                ...
                                if (doc["vol_applyDate"].toString()
                                    ...
                                    Toast.makeText(this, "이미 해당 날짜에 봉사가 있습니다.", Toast.LENGTH_SHORT)
                                        .show()
                                    break
                                }
                                ...
                                db.collection("UserData").document(doc.id.toString())
                                    .update(userauth)
                                Toast.makeText(this, "담당자 승인 후 알림을 보내드리겠습니다.", Toast.LENGTH_SHORT)
                                    .show()
                                setAlarm()
                                if (progressDialog.isShowing) {
                                    progressDialog.dismiss()
    ...
    fun setAlarm() {
        ...
        var builder = NotificationCompat.Builder(this, "MY_channel")
            ...
            .setContentText("${vol_title} 봉사가 승인되었습니다!")
            .setAutoCancel(true)
